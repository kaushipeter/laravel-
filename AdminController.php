...
class AdminController extends Controller
{
    ...
    public function messages(Request $request)
    {
        ...
        if ($request->has('search')) {
            ...
            // Vulnerable Raw SQL
            // Beware: 'search' input is not sanitized or bound.
            $messages = \Illuminate\Support\Facades\DB::select("SELECT * FROM messages WHERE name LIKE '%$search%' OR message LIKE '%$search%'");
        } else {
